...
impl From<APIErrorCode> for Status {
    fn from(code: APIErrorCode) -> Status {
        match code {
            APIErrorCode::UnknownAccount => Status::BadRequest,
            APIErrorCode::Unauthorized => Status::Unauthorized,
            APIErrorCode::InvalidFormBody => Status::BadRequest
        }
    }
}
