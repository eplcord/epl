                        ...
                        Some(Box::new(SharedMessage {
                            ...
                            author: if let Some(author_ref) = message_ref.1 {
                                Some(SharedUser {
                                    ...
                                    public_flags: generate_public_flags(get_user_flags(
                                        author_ref.flags,
                                    )),
    ...
    match requested_channel {
        ...
        Some(requested_channel) => {
            ...
            if message.message_reference.is_some() {
                let requested_message = Message::find_by_id(
                    message
                        .message_reference
                        .unwrap()
                        .message_id
            ...
            let new_message = message::ActiveModel {
                ...
                reference_message_id: Set(if let Some(message_ref) = refed_message.clone() {
                    Some(message_ref.0.id)
                } else {
                    None
                }),
                reference_channel_id: Set(if let Some(message_ref) = refed_message.clone() {
